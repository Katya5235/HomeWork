...
yield_factor = float(input('Введите коэффициент урожайности: '))  
list_of_counts_of_berries = [int(yield_factor * i) for i in range(number_of_bushes)] 
...
for i in range(len(list_of_counts_of_berries)):
    count_of_berries_on_left = list_of_counts_of_berries[i]  
    count_of_berries_in_the_middle = list_of_counts_of_berries[(i + 1) % len(list_of_counts_of_berries)]  
    count_of_berries_on_right = list_of_counts_of_berries[(i + 2) % len(list_of_counts_of_berries)]  
